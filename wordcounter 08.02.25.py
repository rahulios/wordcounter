...
Productivity Score: {self.statistics.get_productivity_score():.1f}
    ...
    def show_security_settings(self):
        ...
        # Main frame
        main_frame = ttk.Frame(security_window, padding="10")
        main_frame.pack(fill=tk.BOTH, expand=True)
        
        # Title
        title_label = ttk.Label(main_frame, text="Security Settings", font=("Helvetica", 14, "bold"))
        ...
        security_var = tk.BooleanVar(value=self.security_manager.security_enabled)
        security_check = ttk.Checkbutton(
            security_frame, 
            text="Enable Security Features", 
            variable=security_var,
            command=lambda: self.security_manager.set_security_enabled(security_var.get())
        )
        security_check.pack(anchor=tk.W)
        
        # Excluded Applications
        excluded_frame = ttk.LabelFrame(main_frame, text="Excluded Applications", padding="10")
        excluded_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
        
        # Listbox for excluded apps
        listbox_frame = ttk.Frame(excluded_frame)
        listbox_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
        
        excluded_listbox = tk.Listbox(listbox_frame, height=8)
        excluded_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        
        scrollbar = ttk.Scrollbar(listbox_frame, orient=tk.VERTICAL, command=excluded_listbox.yview)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        excluded_listbox.config(yscrollcommand=scrollbar.set)
        
        # Buttons for excluded apps
        excluded_buttons_frame = ttk.Frame(excluded_frame)
        excluded_buttons_frame.pack(fill=tk.X)
        
        def add_excluded_app():
            app_name = simpledialog.askstring("Add Application", "Enter application name (e.g., myapp.exe):")
            if app_name:
        ...
        ttk.Button(excluded_buttons_frame, text="Add Application", command=add_excluded_app).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(excluded_buttons_frame, text="Remove Selected", command=remove_excluded_app).pack(side=tk.LEFT)
        ...
        current_keywords = ", ".join(sorted(self.security_manager.sensitive_keywords))
        keywords_text.insert("1.0", current_keywords)
        
        # Current Context
        context_frame = ttk.LabelFrame(main_frame, text="Current Context", padding="10")
        context_frame.pack(fill=tk.X, pady=(0, 10))
        
        context_label = ttk.Label(context_frame, text="Click 'Update Context' to see current application")
        context_label.pack()
        ...
        def update_context():
            ...
            context_label.config(text=context_text)
        
        ttk.Button(context_frame, text="Update Context", command=update_context).pack(pady=(5, 0))
        ...
        def save_keywords():
            ...
            keywords = [kw.strip() for kw in keywords_text_content.split(",") if kw.strip()]
            self.security_manager.sensitive_keywords = set(keywords)
            messagebox.showinfo("Success", "Sensitive keywords updated successfully!")
        ...
        ttk.Button(buttons_frame, text="Save Keywords", command=save_keywords).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(buttons_frame, text="Save All", command=save_settings).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(buttons_frame, text="Close", command=close_dialog).pack(side=tk.RIGHT)
